...
class GRUGating(nn.Module):
    ...
    def forward(self, x, residual):
        ...
        gated_output = self.gru(x, residual)
...
class LowRank(nn.Module):
    def __init__(self, embed_dim, att_type, att_heads, att_mid_dim, att_mid_drop):
        ...
        self.m_k = nn.Parameter(torch.FloatTensor(1, cfg.MODEL.MEMORY_NUM, embed_dim))
        self.m_v = nn.Parameter(torch.FloatTensor(1, cfg.MODEL.MEMORY_NUM, embed_dim))
        nn.init.normal_(self.m_k, 0, 1 / self.head_dim)
        nn.init.normal_(self.m_v, 0, 1 / cfg.MODEL.MEMORY_NUM)
    ...
    def forward2(self, query, key, mask, value1, value2, precompute=False):
        ...
        mask = torch.cat((mask, torch.ones(mask.size(0), mask.size(1), cfg.MODEL.MEMORY_NUM).to(mask)), dim=-1)
