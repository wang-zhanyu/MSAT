...
class XTransformer(BasicModel):
    ...
    def forward(self, **kwargs):
        ...
        gx, encoder_out, mlc_logit, att_mask = self.encoder(att_feats, att_mask)
        decoder_out = self.decoder(gx, seq, encoder_out, att_mask, seq_mask)
        return decoder_out, mlc_logit
    ...
    def decode_beam(self, **kwargs):
        ...
        att_feats = self.att_embed(att_feats)
        att_mask = att_mask[:, :att_feats.shape[1]]
        gx, encoder_out, _, att_mask = self.encoder(att_feats, att_mask)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.mlc_embeder = MLCEncoderLayer(
            embed_dim=embed_dim,
            dropout=dropout,
            att_type=att_type,
            att_heads=att_heads,
        ...
        self.proj_norm_mlc = nn.Sequential(
            nn.Linear(embed_dim * (layer_num), embed_dim),
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers[:-1]:
            ...
            gx_arr.append(gx)
        mlc_gx = torch.cat(gx_arr, dim=-1)
        mlc_gx = self.proj_norm_mlc(mlc_gx)
        mlc_gx = self.mlc_embeder(mlc_gx, x, mask)
        mlc_gx = self.dropout_lm(mlc_gx)
        mlc_out = self.generator(mlc_gx)

        gx, x = self.layers[-1](gx, x, mask)
        gx_arr.append(gx)
...
class MLCEncoderLayer(nn.Module):
    def __init__(
            self,
            embed_dim,
            dropout,
        ...
        super(MLCEncoderLayer, self).__init__()
        self.encoder_attn = LowRank(
            embed_dim=embed_dim,
            att_type=att_type,
            att_heads=att_heads,
